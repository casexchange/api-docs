...
curl -H "X-API-Key: YOUR_API_KEY" \
  https://api.casexchange.com/firms/me
```
...
| `isInternal` | `boolean` | Whether the firm is an internal CaseXchange firm |
...
```bash
curl -H "X-API-Key: YOUR_API_KEY" \
  "https://api.casexchange.com/firms/available?page=2&limit=50&search=jones"
```
