...
| **Internal API** (`/api/v1/...`) | JWT Bearer token | `Authorization: Bearer <jwt>` | CaseXchange dashboard, API key management, OAuth client registration |
...
- OAuth client registration via `POST /api/v1/admin/oauth-clients`
...
JWT authentication is used by the CaseXchange web dashboard and by server integrations that need to manage API keys or OAuth clients.
...
curl https://api.casexchange.com/api/public/v1/firms/me \
  -H "X-API-Key: cxp_std_live_abc123..."
```
